...
.PARAMETER Org
    The org/org id this edge belongs to.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File deploy\install-edge.ps1 `
        -Central https://central.example.net -Token s3cret -Org ispA -Node edge-w1
#>
...
"@ | Set-Content -Path $launcher -Encoding ASCII
    ...
    Write-Host ''
    Ok "WISP edge (fleet path) installed. Node $Org/$Node -> $Central."
