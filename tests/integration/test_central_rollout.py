"""Staged rollout / version-authority tests (Phase 10 Part D): the canary→promoted→done
(or halted) state machine, and the per-node update directive central hands back in the
heartbeat reply. Temp store + injected clock; no network, no real binaries."""
import os
import sys
import tempfile
import unittest
...
class RolloutTest(unittest.TestCase):
    ...
    def test_canary_node_gets_directive_others_dont(self):
        ...
        self.assertIsNone(rollout.directive_for(self.store, "ispA", "edge-2", "v1",
                                                "linux-amd64", now=NOW))  # not in canary
    ...
    def test_canary_promotes_when_canary_healthy_on_target(self):
        self._node("edge-1", "v2")            # canary already updated + fresh
    ...
    def test_canary_stays_until_healthy(self):
        self._node("edge-1", "v1")            # canary not yet updated
        self.store.set_rollout("ispA", "v2", ["edge-1"], now=_iso(NOW))
    ...
    def test_canary_halts_past_window_when_unhealthy(self):
        self._node("edge-1", "v1")            # canary failed to update
        self.store.set_rollout("ispA", "v2", ["edge-1"], now=_iso(NOW))
        later = NOW + timedelta(seconds=700)  # past the 600s window
    ...
    def test_canary_halts_when_canary_goes_silent(self):
        self._node("edge-1", "v2", age_s=900)  # updated but heartbeat now stale (came up, died)
        self.store.set_rollout("ispA", "v2", ["edge-1"], now=_iso(NOW))
