...
class DeviceRollupTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dev = self.store.create_org_device(TENANT, {
            ...
            "region": None, "parent_device_id": None})
        self.eng = central_engine.build_engine(self.store, TENANT, self.cfg)
    ...
    def test_record_cycle_folds_into_the_hour_bucket(self):
        ...
        cycle = central_engine.run_cycle(self.store, TENANT, self.eng, results, ts)
        central_rollup.record_cycle(self.store, TENANT, self.eng, cycle, results, ts)

        since, until = "2026-01-01T00:00:00", "2026-01-01T23:59:59"
        buckets = self.store.device_rollup_series(TENANT, self.dev, since, until)
    ...
    def test_multiple_samples_average_within_the_same_bucket(self):
        ...
        buckets = self.store.device_rollup_series(
            TENANT, self.dev, "2026-01-01T00:00:00", "2026-01-01T23:59:59")
        self.assertEqual(len(buckets), 1)
    ...
    def test_prune_removes_only_buckets_older_than_retention(self):
        ...
        buckets = self.store.device_rollup_series(TENANT, self.dev, "2000-01-01T00:00:00",
                                                   "2030-01-01T00:00:00")
        self.assertEqual(len(buckets), 1)
