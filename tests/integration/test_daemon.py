"""Daemon glue tests: the poll-gather error policy + concurrency bound.
...
100% loss so one bad host never sinks the whole cycle. Also guards the semaphore
that bounds probe fan-out, since past `ulimit -n` an unbounded `gather` reads as a
fake mass outage.

`_gather_pings` is the one piece of the old standalone/Phase-10 daemon still shared
with central-brain mode (see `apps/daemon/main.py`); the FSM-confirmation tests that
used to live here (`ConfirmDown`/`ConfirmUp`/`BetweenCycleWatch`) moved to central
along with the FSM — see `tests/integration/test_central_brain.py`.
"""
import asyncio
