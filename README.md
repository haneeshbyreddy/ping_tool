...
# / is the marketing landing page; the dashboard is at /app — log in there and
# add your first device from the Network page

python -m unittest discover -s tests   # 352 tests, pure stdlib
```
...
The dashboard itself (`central/static/`) is a built React SPA served at `/app` — `./run.sh`
serves the already-committed build, so the quick start above needs no Node. `/` is a
self-contained marketing landing page (`web/public/landing.html`, copied into `static/` on
build). Only touch the frontend source if you're changing the dashboard: `cd web && npm
install && npm run build` regenerates `central/static/` (Node 20+; build-time only, never a
runtime dependency).

## Central dashboard

React + Tailwind + shadcn/ui (source in `web/`, see `CLAUDE.md` for the architecture).
Pages:
...
deploy/      # wisp-edge.service, build-deb.sh, wisp-edge-setup.iss, PyInstaller spec
...
```
Install: the `wisp-edge` **.deb** (amd64/arm64) on Linux, the **setup .exe** on
Windows — both built by CI per release; see "Going live" below. These are the only
supported install path (no separate single-box mode). The release notes link exactly
those files; every other asset on the release page (bare `wisp-edge-*` /
`wisp-supervisor-*` binaries, `manifest.json`) is the self-update channel that
installed probes fetch automatically — never download those by hand. CI
(`.github/workflows/release.yml`) builds+tests every push/PR; on a `v*` tag it also
signs (Authenticode on Windows binaries + the setup exe, minisign over `SHA256SUMS`) —
no-ops until `WINDOWS_CODESIGN_PFX`/`MINISIGN_KEY` secrets are set, so unsigned
forks/PRs still build.
...
| `WISP_SNMP_WALK_TIMEOUT_S` | `20` | wall-clock cap on one device's whole IF-MIB walk |
| `WISP_SNMP_MAX_INFLIGHT` | `4` | concurrent device walks per SNMP sweep |
...
1. On the edge box, install the release package for its platform. The easy path:
   register the probe in the dashboard (Network → Probes → Register) and follow the
   per-OS instructions it shows — download link + every value pre-filled. By hand:
   ```bash
   # Linux (amd64/arm64) — the .deb from the GitHub Release (version-less asset
   # name, so /releases/latest/download/ always serves the newest):
   sudo dpkg -i wisp-edge-linux-amd64.deb
   sudo vi /etc/wisp/edge.env      # central URL + enrollment token + org
   sudo systemctl enable --now wisp-edge
   ```
   Windows: run `wisp-edge-setup-win-amd64.exe` (elevated) and fill in the wizard, or
   fleet-silent: `wisp-edge-setup-win-amd64.exe /VERYSILENT /Central=… /Token=…
   /Org=… /Node=…`. Both packages install the agent + supervisor under `/opt/wisp`
   (`Program Files\WISP` on Windows), keep identity/config in `/etc/wisp`
   (`ProgramData\WISP`) untouched by updates, enable unprivileged ICMP (Linux), and run
   the **supervisor**, not the agent directly, so the agent can self-update. On Windows
   there's no unprivileged-ICMP path (no ping-group, no datagram sockets), so icmplib
   needs raw sockets — that's why the Scheduled Task runs as SYSTEM (hidden). The
   human-visible half is **`wisp-tray.exe`** in the notification area ("Show hidden
   icons"): live status (green = reporting, red = broken, with the reason), open
   dashboard/logs, start/restart, uninstall, and Exit-and-stop. The setup **verifies**
   the probe actually reports before finishing — if it can't, it says so and points at
   `C:\ProgramData\WISP\install.log` + `logs\edge.log` (the probe also drops
   machine-readable `status.json` there every cycle). Not verified on real Windows
