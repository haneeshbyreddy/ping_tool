...
// GET /api/events (server.py:292) is an org-scoped SSE stream that pushes a cheap
...
// behavior) if the org scope is stable; rebuilt whenever it changes.
...
export function useEventStream(org: string | null) {
  ...
  useEffect(() => {
    const source = new EventSource(`/api/events${tq(org)}`)
