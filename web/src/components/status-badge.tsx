...
export function StatusDot({ tone }: { tone: keyof typeof TONE_CLASS }) {
  ...
  return <span className={cn("size-1.5 shrink-0 rounded-full", dotClass[tone])} />
}
...
export function StateBadge({ state, label }: { state: string | null | undefined; label?: string }) {
  ...
  return (
    <span className={cn(
      "inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[11px] font-bold capitalize",
      TONE_CLASS[tone],
    )}>
      <StatusDot tone={tone} />
...
export function TonePill({ tone, children }: { tone: keyof typeof TONE_CLASS; children: ReactNode }) {
  return (
    <span className={cn(
      "inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[11px] font-bold whitespace-nowrap",
      TONE_CLASS[tone],
    )}>
      {children}
    </span>
